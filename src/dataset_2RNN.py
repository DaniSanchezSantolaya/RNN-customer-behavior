...
class DataSet():
    ...
    def __init__(self, X_train, X_train_events, Y_train, X_val, X_val_events, Y_val):
        ...
        self._X_train_events = X_train_events
        self._Y_train = Y_train
        self._X_val = X_val
        self._X_val_events = X_val_events
        self._Y_val = Y_val
    ...
    def next_batch(self, batch_size):
        ...
        if self._index_in_epoch > self._num_examples:
            ...
            self._X_train_events = self._X_train_events[perm]
        ...
        X_events = self._X_train_events[start:end]
        ...
        batch_x_events = []
        batch_y = []
        for x,x_event,y in zip(X,X_events,Y):
            batch_x.append(x.toarray())
            batch_x_events.append(x_event.toarray())
            batch_y.append(y.toarray().reshape(n_output))
        return np.array(batch_x), np.array(batch_x_events), np.array(batch_y)