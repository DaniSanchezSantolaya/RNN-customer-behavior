...
#python experiment_santander_m1.py load_pickles p_val seq_length opt learning_rate n_hidden batch_size rnn_type max_steps
#python experiment_santander_m1.py True 0.20 17 adam 0.0001 64 128 lstm 1 0.1 output_type 50000
#C:\Projects\Thesis\src\baseline_model>python experiment_santander_m3.py True 0.20 17 adam 0.0001 64 128 lstm2 2 0 sigmoid 500000
...
type_output = sys.argv[11]
max_steps = int(sys.argv[12])

name_submission = 'kaggle_submissions/m1-' + 'interactions_' + str(seq_length) + '-' + opt + '-lrate_' + str(learning_rate) + '-hidden_' + str(n_hidden) + '-bSize_' + str(batch_size) + '-' + rnn_type + '-rnn_layers_' + str(rnn_layers) + '-dropout_' + str(dropout) + '-typeoutput_' + str(type_output) + '-max_steps_' + str(max_steps) + '.csv'
